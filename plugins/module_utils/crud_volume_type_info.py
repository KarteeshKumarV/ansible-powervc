...
"""
This module performs the Get Volume Type Details operation.
"""
...
def get_volume_type_details(authtoken, volume_type_url):
    """
    Performs Get Volume Type Details operation on the Volume Type ID passed
    """
    headers_scg = get_headers(authtoken)
    responce = requests.get(volume_type_url, headers=headers_scg, verify=False)
    if responce.ok:
        return responce.json()


def volume_ops(mod, connectn, authtoken, tenant_id, vol_type_id):
    service_name = "volume"
    endpoint = get_endpoint_url_by_service_name(mod, connectn, service_name, tenant_id)
    if vol_type_id:
        volume_type_url = f"{endpoint}/types/{vol_type_id}"
