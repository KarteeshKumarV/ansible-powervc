...
def get_collocation_rules_id(
    ...
    endpoint_compute = get_endpoint_url_by_service_name(
        mod,
        connectn,
        service_name,
        tenant_id
...
def server_flavor(
    ...
    volume_id = responce.json()['image']['metadata'][
    ...
    flavor_details = {
        "ram": responce.json()["flavor"]["ram"],
        "vcpus": responce.json()["flavor"]["vcpus"],
        "disk": size
    }
...
def create_vm(headers_vm, vm_url, data, vm_name):
    ...
    if responce.ok:
        ...
        return (
            f"VM '{vm_name}' create operation is done",
            responce.json()
        )
        ...
        return (
            f"VM '{vm_name}' create operation failed",
            responce.json()
        )
...
def delete_vm(headers_vm, vm_url, vm_name):
    ...
    if responce.ok:
        ...
        return (
            f"VM '{vm_name}' has been removed/deleted"
        )
